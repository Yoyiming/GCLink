...
parser.add_argument('-sample', type=str, default='sample1',
                    help='sample')
parser.add_argument('-cell_type', type=str, default='mHSC-L',
                    help='cell_type')
...
class My_Model(nn.Module):
    ...
    def forward(self, data_feature, adj1, adj2, train_data):
...
def train(model, contrast_model, optimizer):
    ...
    for train_x, train_y in DataLoader(train_load, batch_size=args.batch_size, shuffle=True):
        ...
        embed1, tf_embed1, target_embed1, pred1, embed2, tf_embed2, target_embed2, pred2 = model(data_feature, adj1, adj2, train_x)
...
encoder_model2 = GENELink(input_dim=feature.size()[1],
                 hidden1_dim=args.hidden_dim[0],
                 hidden2_dim=args.hidden_dim[1],
                 hidden3_dim=args.hidden_dim[2],
                 output_dim=args.output_dim,
...
for epoch in range(args.epochs):
    ...
    model.eval()
    _, _, _, _, _, _, _, score = model(data_feature, adj1, adj2, validation_data)
    if args.flag:
    ...
    if AUC > AUC_Threshold:
        AUC_Threshold = AUC
        torch.save(model.state_dict(), model_path + '/' + args.cell_type + '1000_unshared_False' + '.pkl')

model.load_state_dict(torch.load(model_path + '/' + args.cell_type + '1000_unshared_False' + '.pkl'))
model.eval()

_, _, _, _, _, _, _, score = model(data_feature, adj1, adj2, test_data)
...
np.savetxt('Result/Specific/TF+1000/' + args.sample + '_' + args.cell_type + '_f+e_unshared_False.txt', np.array([AUC, AUPR]), fmt='%.3f')

# y_p = score[:, -1]
# y_p = y_p.cpu().detach().numpy()
# y_p = list(y_p.flatten())
# name = ['predict']
# pre_data = pd.DataFrame(columns=name, data=y_p)

# pre_data.to_csv("mESC_pre.csv")
