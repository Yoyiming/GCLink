...
parser.add_argument('-sample', type=str, default='sample2',
                    help='sample')
...
for epoch in range(args.epochs):
    ...
    #     AUC_Threshold = AUC
    #     torch.save(model.state_dict(), model_path + '/' + args.cell_type + '1000_i+e_shared_False' + '.pkl')
    print('Epoch:{}'.format(epoch + 1), 'train loss:{:.5F}'.format(running_loss))
torch.save(model.state_dict(), model_path + '/' + args.cell_type + '1000_i+e_shared_False' + '.pkl')

model.load_state_dict(torch.load(model_path + '/' + args.cell_type + '1000_i+e_shared_False' + '.pkl'))
model.eval()
...
# np.savetxt('Result/Specific/TF+1000/' + args.sample + '_' + args.cell_type + '_i+e_shared_False_contrast.txt', np.array([AUC, AUPR]), fmt='%.3f')
