...
parser.add_argument('-sample', type=str, default='sample1',
                    help='sample')
parser.add_argument('-cell_type', type=str, default='mHSC-E',
                    help='cell_type')
...
class My_Model(nn.Module):
    ...
    def forward(self, data_feature, adj, train_data):
        ...
        w = degree_drop_weights(index).to(device)
        edge_index2 = drop_edge_weighted(index, w, p=0.3, threshold=0.5)
...
def drop_edge_weighted(edge_index, edge_weights, p, threshold):
...
for epoch in range(args.epochs):
    ...
    model.eval()
    _, _, _, _, _, _, _, score = model(data_feature, adj, validation_data)
    if args.flag:
    ...
    if AUC > AUC_Threshold:
        AUC_Threshold = AUC
        torch.save(model.state_dict(), model_path + '/' + args.cell_type + '1000_centrality_False' + '.pkl')

model.load_state_dict(torch.load(model_path + '/' + args.cell_type + '1000_centrality_False' + '.pkl'))
model.eval()
...
np.savetxt('Result/Specific/TF+1000/' + args.sample + '_' + args.cell_type + 'centrality_False.txt', np.array([AUC, AUPR]), fmt='%.3f')
