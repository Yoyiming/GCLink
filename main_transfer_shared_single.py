...
parser.add_argument('-sample', type=str, default='sample1',
                    help='sample')
parser.add_argument('-cell_type', type=str, default='mESC',
                    help='cell_type')
...
def main_function():
    ...
    torch.save(model.state_dict(), model_path + '/' + args.cell_type + '1000_' + '_no_aug_finetune_shared.pkl')
...
model.load_state_dict(torch.load(model_path + '/source_mESC1000_i+e_True_shared.pkl'))
...
model.load_state_dict(torch.load(model_path + '/' + args.cell_type + '1000_' + '_no_aug_finetune_shared.pkl'))
...
np.savetxt('AUC_AUPR_Result/Specific/TF+1000/mESC1000_' + args.sample + '_' + args.cell_type + '_transfer_no_aug_finetune.txt', np.array([AUC, AUPR]), fmt='%.3f')
