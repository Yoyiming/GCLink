...
class scRNADataset(Dataset):
    ...
    def Adj_Generate(self, TF_set, direction=False, loop=False):
        ...
        adj = sp.dok_matrix((self.num_gene, self.num_gene),
                            dtype=np.float32)  # 创建具有初始形状 (N,N) 的稀疏矩阵, N=feature_shape[0]
        ...
        if loop:
            # identity产生单位对角矩阵，但是返回的是稀疏的存储内容
            adj = adj + sp.identity(self.num_gene)

        adj = adj.todok()  # 返回稀疏矩阵的dok_matrix形式
