...
parser.add_argument('-sample', type=str, default='sample1',
                    help='sample')
parser.add_argument('-cell_type', type=str, default='mDC',
                    help='cell_type')
...
def train(model, contrast_model, optimizer):
    ...
    for train_x, train_y in DataLoader(train_load, batch_size=args.batch_size, shuffle=True):
        ...
        con_loss = contrast_model(h1=embed1, h2=embed2, batch=args.batch_size)
...
def main_function():
    ...
    torch.save(model.state_dict(), model_path + '/' + args.cell_type + '500_' + str(pe) + '_finetune.pkl')
...
# 分层抽样，抽取5%作为验证，5%作为训练
# x = train_data.drop('Label', axis=1)
# y = train_data['Label']
# validation_data, train_data, valid_label, train_label = train_test_split(x, y, test_size=0.9, stratify=y, random_state=42)
...
model.load_state_dict(torch.load(model_path + '/mESC1000.pkl'))
...
pe = 0.3
aug1 = A.Identity()
aug2 = A.EdgeRemoving(pe)
main_function()

model.load_state_dict(torch.load(model_path + '/' + args.cell_type + '1000_' + str(pe) + '_finetune.pkl'))
model.eval()
...
np.savetxt('AUC_AUPR_Result/Specific/TF+1000/mESC1000_' + args.sample + '_' + args.cell_type + '_no_finetune_e+i.txt', np.array([AUC, AUPR]), fmt='%.3f')
